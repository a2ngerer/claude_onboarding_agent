...
- Use `Tool(param:value)` permission rules (e.g. `Agent(model:opus)`) to restrict which model tiers subagents may use — set in `.claude/settings.json` or managed settings.
