...
- Use `isolation: "worktree"` on the `Agent` tool call when the subagent needs to write without affecting the working copy; the worktree is auto-cleaned if no changes are made.
...
## Background agents vs. subagents vs. agent teams

| Mode | Scope | Communication | Use when |
|---|---|---|---|
| **Subagent** (Agent tool) | Within current session | Returns result to parent | Side tasks in an active session |
| **Background agent** (`claude agents`) | Independent session | None — runs silently | Long-running tasks while you do other work |
| **Agent team** | Multiple sessions | Sessions communicate | Workflows requiring inter-agent coordination |

Background agent notes:
- Pinned sessions stay alive when idle; updates restart them in-place.
- File attachments are passed as actual content (not `[Pasted text]` placeholders).
- `worktree.bgIsolation: "none"` in settings lets background agents edit the working copy directly instead of a worktree.
- Launch a background shell session: `claude --bg --exec '<command>'`

## Dynamic workflows

Ask Claude to create a workflow that orchestrates tens to hundreds of agents in parallel. View runs with `/workflows`. Use for large-scale parallel tasks (batch evaluations, full-codebase analysis). Do not use for tasks that fit in a single session.
...
- Set `worktree.baseRef: "fresh"` (default) to branch from the remote default, or `"head"` to branch from local HEAD.
...
- Use `/agents` to create, edit, and manage subagents interactively within a session.
