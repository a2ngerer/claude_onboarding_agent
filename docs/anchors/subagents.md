...
- Gate model costs at the caller: `Agent(model:opus)` in a `deny` permission rule blocks subagents from dispatching on Opus; the same `Tool(param:value)` syntax works for any model alias.
