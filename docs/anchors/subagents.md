...
- Use `Agent(model:opus)` (or `haiku`, `sonnet`) in `permissions.allow` to gate tool access by the spawned model — e.g. allow a destructive tool only for Opus-tier subagents.
