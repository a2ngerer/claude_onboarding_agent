...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Key CLI operations: `rename`, `daily:path` (open/create daily note), `search:context` (contextual search). Reference: `.claude/agents/obsidian-vault-keeper.md`.
