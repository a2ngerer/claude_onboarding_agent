...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (v1.12+ ships a bundled binary replacing the Electron method; significantly faster terminal interactions). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`. Key CLI commands: `rename` (renames a note and updates all backlinks), `search:context` (full-text search with surrounding context), `daily:path` (resolve the path of the daily note without opening the app).
