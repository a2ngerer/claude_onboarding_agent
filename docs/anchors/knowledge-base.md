...
Reserved Obsidian keys: `aliases`, `tags`, `cssclasses`. Publish-only reserved keys: `publish`, `permalink`, `description`, `image`/`cover`. Dates use `YYYY-MM-DD` or ISO-8601 with time (`YYYY-MM-DDTHH:MM:SS`); lists may use YAML block or flow syntax.
...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (v1.12+ adds `daily:path`, `rename`, `search:context` commands). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
