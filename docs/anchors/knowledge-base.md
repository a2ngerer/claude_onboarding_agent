...
## Obsidian CLI notes (v1.12+)

Key commands used by the vault-keeper agent: `open`, `read`, `write`, `search`, `search:context` (returns surrounding context), `rename`, `daily`, `daily:path`, `tags:list`. Recent default changes: the `all` parameter now defaults to `active`; `silent` now defaults to `open`. `daily:prepend` inserts content after frontmatter rather than at the file start. The CLI ships as a standalone binary (not Electron-wrapped) in recent releases — significantly faster for scripted vault ops.
