...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Use the CLI's `rename` command (Obsidian v1.13+) to rename notes while preserving all backlinks. Reference: `.claude/agents/obsidian-vault-keeper.md`.
