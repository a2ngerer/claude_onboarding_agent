...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (available since v1.12.4). Key CLI commands: `daily:path` (get expected daily-note path), `rename` (rename with backlink rewrite), `search:context` (search with surrounding context). Forbids direct `Edit`/`Write` so wikilinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
