...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (v1.13+ includes a `rename` command that rewrites backlinks automatically). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
