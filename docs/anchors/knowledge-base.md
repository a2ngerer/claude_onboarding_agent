...
**Obsidian 1.9+:** The deprecated singular-form keys (`tag`, `alias`, `cssclass`) are no longer supported. Use `tags`, `aliases`, and `cssclasses`. Run the built-in Format Converter for bulk vault updates.
...
- Using deprecated property keys (`tag`, `alias`, `cssclass`) — unsupported in Obsidian 1.9+; they are silently ignored.
