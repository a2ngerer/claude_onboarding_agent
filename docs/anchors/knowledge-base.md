...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`. As of Obsidian v1.12.7, the CLI is a dedicated binary (not Electron-based) with built-in command autocompletion; update any install paths that referenced the older Electron binary.
