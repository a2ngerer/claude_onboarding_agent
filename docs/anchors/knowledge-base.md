...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (v1.12+, dedicated binary). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Key CLI commands: `daily:path` (open daily note path), `rename` (rename with link-rewrite), file operations via `id=` addressing. Reference: `.claude/agents/obsidian-vault-keeper.md`.
