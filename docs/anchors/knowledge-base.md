...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (bundled with the desktop installer since 1.12.0; no separate install needed). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
