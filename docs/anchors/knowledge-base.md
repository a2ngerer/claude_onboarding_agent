...
Reserved Obsidian keys: `aliases`, `tags`, `cssclasses`. **Deprecated since Obsidian 1.9:** use `tags` not `tag`, `aliases` not `alias`, `cssclasses` not `cssclass` — the old singular forms break dataview queries. Dates use `YYYY-MM-DD` or ISO-8601 with time; lists may use YAML block or flow syntax.
...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. The CLI is bundled with the Obsidian installer since v1.12.0 (no separate install needed). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
...
- Using deprecated property keys (`tag`, `alias`, `cssclass`) — switch to the plural forms to stay compatible with Obsidian 1.9+.
