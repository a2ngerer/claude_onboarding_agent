...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. As of v1.12.0 the CLI ships a native bundled binary, significantly faster than the prior Electron-based launcher. Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
