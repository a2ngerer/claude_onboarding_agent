...
Reserved Obsidian keys: `aliases`, `tags`, `cssclasses`. Dates use `YYYY-MM-DD` or ISO-8601 with time; lists may use YAML block or flow syntax. **Deprecated since Obsidian 1.9+:** the singular forms `tag`, `alias`, `cssclass` — rename to their plural equivalents (`tags`, `aliases`, `cssclasses`).
