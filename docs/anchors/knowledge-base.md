...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI. Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent; use the CLI `rename` command (v1.12.4+) for safe renames that rewrite backlinks automatically. Reference: `.claude/agents/obsidian-vault-keeper.md`.
