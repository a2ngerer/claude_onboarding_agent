...
Deprecated singular forms (Obsidian 1.4+): `tag` → `tags`, `alias` → `aliases`, `cssclass` → `cssclasses`.
...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (enhanced in v1.13.0 with command autocompletion and help). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
