...
Reserved Obsidian keys: `aliases`, `tags`, `cssclasses`. Obsidian also supports `date & time` (stored as `YYYY-MM-DDTHH:MM:SS`), `number`, and `checkbox` property types. Dates use `YYYY-MM-DD` or ISO-8601 with time; lists may use YAML block or flow syntax.
...
- **Vault keeper** — dispatched subagent owning all vault I/O via the Obsidian CLI (commands include `rename`, `daily:path`, `search:context`). Forbids direct `Edit`/`Write` so wikilinks and backlinks stay consistent. Reference: `.claude/agents/obsidian-vault-keeper.md`.
