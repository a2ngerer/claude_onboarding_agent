...
> Starting with Claude 4.6+, dateless model IDs are **pinned snapshots**, not evergreen pointers. Aliases for current-generation models resolve to the same pinned ID, not to "latest".
...
- Prefer the **pinned ID** over an alias in production — even dateless IDs are pinned, but names may shift with future generations.
- Opus 4.8 uses **adaptive thinking** (effort defaults to `high`; set `effort` explicitly to override). It does **not** support extended thinking.
- Sonnet 4.6 supports both **extended thinking** and **adaptive thinking**.
