...
**Capabilities:** Opus 4.7 supports adaptive thinking; Sonnet 4.6 and Haiku 4.5 support extended thinking. Opus 4.7 uses a new tokenizer that increases token count 1.0–1.35× versus prior models at the same input.
...
- `claude-sonnet-4-20250514` (alias `claude-sonnet-4-0`) → replace with `claude-sonnet-4-6`
- `claude-opus-4-20250514` (alias `claude-opus-4-0`) → replace with `claude-opus-4-7`

**Previously deprecated (Claude 3.x and earlier):**
...
- Prefer the **dated ID** (e.g. `claude-haiku-4-5-20251001`) over a dateless alias in production — dateless aliases are pinned snapshots for the 4.6+ generation but may be re-pointed to newer snapshots in the future.
- When migrating between model families, re-run your eval suite; prompts tuned for one generation may need light adjustment.
- For Opus 4.7 migrations: account for the new tokenizer (1.0–1.35× more tokens per identical input at unchanged pricing).
