...
Note: Opus 4.7 uses a new tokenizer — effective token costs are ~1.46× higher for text than the 4.6 generation.
...
- Starting with the 4.6 generation, model IDs are **pinned snapshots** — the alias equals the model ID, there are no `-latest` evergreen pointers. Use the explicit ID in production.
- When migrating between versions, re-run your eval suite; prompts tuned for one family may need light adjustment.
- Opus 4.7 supports `xhigh` effort level (between `high` and `max`) for maximum reasoning depth.
