...
- Prefer the **dated ID** over aliases in production. Starting with the Claude 4.6 generation, dateless IDs (e.g. `claude-sonnet-4-6`) are also pinned snapshots, not evergreen pointers.
- Claude Opus 4.8 defaults to `effort: high` on all surfaces; set it explicitly to use a lower effort level.
