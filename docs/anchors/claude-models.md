...
Opus 4.7 uses a new tokenizer that requires ~1.46× the token count of 4.6 for identical text — factor this into cost estimates when migrating.
...
- **Claude Mythos Preview** — invitation-only research preview for defensive cybersecurity workflows; not available via API self-serve (Project Glasswing, Anthropic).

## Tips

- Prefer the **dated ID** (`claude-haiku-4-5-20251001`) over the alias (`claude-haiku-4-5`) in production — the alias may advance to a newer snapshot without notice.
- Opus 4.7's new tokenizer means identical prompts cost more tokens than on 4.6; re-run cost estimates before migration.
