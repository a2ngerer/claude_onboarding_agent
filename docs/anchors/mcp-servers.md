...
- **Authentication:** `claude mcp login <name>` opens a browser OAuth flow for a named remote server; `claude mcp logout <name>` revokes credentials. Use `--no-browser` for SSH sessions (redirects to stdin).
