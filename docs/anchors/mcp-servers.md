...
- Set `"alwaysLoad": true` on a server entry to skip tool-search deferral — tools load immediately rather than waiting for Claude to decide they're needed. Use only for small, always-relevant servers; it adds overhead to every turn.
