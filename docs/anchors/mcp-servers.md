...
Browse reviewed connectors at `claude.ai/directory`. Keep the set small: every installed MCP expands the tool-selection surface and the trust boundary.

## Transports

| Transport | Status | Use when |
|-----------|--------|----------|
| `http` | ✅ Recommended | Remote servers — most cloud services |
| `stdio` | ✅ Supported | Local processes, scripts needing direct system access |
| `ws` | ✅ Supported | Servers that push events to Claude unprompted |
| `sse` | ⚠️ Deprecated | — migrate to `http` |
...
- **github** — issues, PRs, reviews via the GitHub API. Install: `claude mcp add --transport http github https://api.githubcopilot.com/mcp/ --header "Authorization: Bearer <TOKEN>"`
...
- **MCP Tool Search is on by default:** tool schemas are deferred and discovered on demand — adding many servers has minimal context impact at session start.
- Use `alwaysLoad: true` in a server's config entry to force its tools into context at startup (for tools Claude needs on every turn).
- For scaffolding new MCP servers, install the `mcp-server-dev` plugin: `/plugin install mcp-server-dev@claude-plugins-official`.
