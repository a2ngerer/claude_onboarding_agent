...
- Use `alwaysLoad: true` in a server's settings entry to exempt it from tool-search deferral, ensuring its tools are always available without an explicit `@`-mention.
