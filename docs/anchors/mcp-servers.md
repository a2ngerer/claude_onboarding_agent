...
- Set `"alwaysLoad": true` in a server's config entry to have all its tools always available, bypassing tool-search deferral.
- `workspace` is a reserved server name — do not use it for custom server entries.
- Browse the official Anthropic MCP registry at `https://api.anthropic.com/mcp-registry/docs` to discover vetted community servers.
