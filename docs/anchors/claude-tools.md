...
Top-level keys in `.claude/settings.json`: `permissions`, `env`, `hooks`, `mcpServers`, `model`, `agent`, `outputStyle`, `sandbox`, `claudeMdExcludes`, `effortLevel`, `worktree`, `autoMode`. Permission rules evaluate in order `deny → ask → allow`; first match wins.
...
| `PreCompact` | Block or prepare for context compaction | Flush notes before compaction |
...
Many more events exist: `SubagentStart`, `SubagentStop`, `UserPromptExpansion`, `PostToolBatch`, `FileChanged`, `InstructionsLoaded`, `Elicitation`, and others — see the hooks reference.

Hooks live in `.claude/settings.json` under `hooks.<EventName>[]` with a `matcher` and a list of handler entries. Handler types: `command` (shell script), `http` (POST to endpoint), `mcp_tool` (invoke MCP tool directly), `prompt` (single-turn LLM eval), `agent` (subagent verification). Plugins ship hooks in `hooks/hooks.json`.

## Slash commands / Skills

- The preferred format is **skills**: `SKILL.md` files in `.claude/skills/<name>/` (or `~/.claude/skills/<name>/`). Skills support frontmatter (`description`, `context`, `agent`, `allowed-tools`, `effort`), sibling supporting files, and path-scoped activation.
- Legacy `.claude/commands/*.md` files still work but are deprecated in favor of the skills format.
...
- Components sit at the plugin root, not inside `.claude-plugin/`: `skills/`, `agents/`, `commands/`, `hooks/`, `.mcp.json`, `.lsp.json`, `monitors/`, `settings.json`, `bin/`, `themes/`.
...
- Do not use `.claude/commands/*.md` for new functionality — prefer `skills/<name>/SKILL.md` which supports frontmatter, sibling files, and invocation control (`disable-model-invocation`, `context: fork`).
