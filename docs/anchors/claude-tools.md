...
- **`requiredMinimumVersion`** / **`requiredMaximumVersion`** — semver bounds; Claude Code refuses to run outside this version range (`requiredMaximumVersion` is managed-only).
...
Hooks live in `.claude/settings.json` under `hooks.<EventName>[]` with a `matcher` and handler entries. Handler types: `command`, `http`, `mcp_tool`, `prompt` (LLM eval), `agent` (subagent verify); add `async: true` for background execution. Plugins ship hooks in `hooks/hooks.json`.

Additional events (not in table above): `SubagentStop`, `UserPromptExpansion`, `StopFailure`, `CwdChanged`, `ConfigChange`, `FileChanged`, `TaskCreated`, `TaskCompleted`, `Elicitation`, `PermissionRequest`, `SubagentStart`.
...
- `/workflows` — view running dynamic workflow jobs and their status.
