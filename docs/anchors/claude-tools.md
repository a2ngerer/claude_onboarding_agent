...
Top-level keys in `.claude/settings.json`: `permissions`, `env`, `hooks`, `mcpServers`, `model`, `agent`, `outputStyle`, `sandbox`, `claudeMdExcludes`, `effortLevel`, `skillOverrides`, `worktree`, `autoMode`, `disableAllHooks`. Permission rules evaluate in order `deny → ask → allow`; first match wins.
...
Hook types: `command`, `http`, `mcp_tool`, `prompt`, `agent`. Hooks live in `.claude/settings.json` under `hooks.<EventName>[]` with a `matcher` and a list of entries. Plugins ship hooks in `hooks/hooks.json`.

| Event | Typical use |
|---|---|
| `SessionStart` | Load context, env vars, or set `watchPaths` for `FileChanged` |
| `UserPromptSubmit` | Validate or enrich the user prompt |
| `PreToolUse` | Block or gate a tool call |
| `PostToolUse` | Lint or log after a tool succeeds; `duration_ms` available |
| `PostToolUseFailure` | React to a failed tool call; use `continueOnBlock` to retry |
| `PostToolBatch` | Act after all parallel tool calls in a batch resolve |
| `PreCompact` / `PostCompact` | Inject or capture context around compaction |
| `SubagentStart` / `SubagentStop` | Observe or gate subagent lifecycle; matcher = agent type |
| `Stop` | Cleanup when Claude finishes a turn |
| `StopFailure` | Handle API errors (`rate_limit`, `billing_error`, etc.) |
| `SessionEnd` | Release resources or save artifacts |
| `FileChanged` | React to watched-file edits on disk |

Notable `command` hook fields: `args: string[]` (exec form, no shell quoting), `asyncRewake` (run in background; wake Claude on exit code 2), `terminalSequence` (emit desktop notifications without a controlling terminal).
